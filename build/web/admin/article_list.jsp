                ...
                <table class="article-table">
                    ...
                    <tbody>
                        <c:forEach items="${articleList}" var="a">
                            <tr>
                                <td>${a.id}</td>
                                <%-- Thay thế dòng <td><img ...> cũ bằng khối <td>...</td> này --%>
                                <td>
                                    <c:choose>
                                        <%-- TRƯỜNG HỢP 1: Nếu link ảnh bắt đầu bằng "http" (là link tuyệt đối từ web) --%>
                                        <c:when test="${a.imageUrl.startsWith('http')}">
                                            <img src="${a.imageUrl}" alt="${a.title}"> <%-- Dùng nguyên link gốc --%>
                                        </c:when>

                                        <%-- TRƯỜNG HỢP 2: Ngược lại (là link tương đối như "images/blog1.jpg") --%>
                                        <c:otherwise>
                                            <img src="${pageContext.request.contextPath}/${a.imageUrl}" alt="${a.title}"> <%-- Thêm contextPath vào --%>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td>${a.title}</td>
